...
Before you can generate reports from the data, epitraxr needs a list of diseases to include in the report. Often, you'll have two lists, one for internal reports and one for public reports. Read these files in using the functions `get_internal_disease_list()` and `get_public_disease_list()`.

```{r get-ilist}
internal_disease_list <- "vignette-data/ireport_diseases.csv"
internal_diseases <- get_internal_disease_list(internal_disease_list)
...
public_diseases <- get_public_disease_list(public_disease_list)
...
report <- create_report_annual_counts(
  data = epitrax_data,
  disease_names = internal_diseases$EpiTrax_name
)
...
report <- create_report_annual_counts(
  data = epitrax_data,
  disease_names = public_diseases$EpiTrax_name
)
